/*
 * layerExBtoA.dll: wamsoft's Layer pixel-buffer utility methods --
 * copyRightBlueToLeftAlpha, copyBottomBlueToTopAlpha, fillAlpha,
 * copyAlphaToProvince, clipAlphaRect, overwrapRect, fillByProvince,
 * fillToProvince.
 *
 * Ported from wamsoft's layerExBTOA plugin (Main.cpp), license "same as
 * KiriKiri itself" per its readme.txt. Noble Works' GFX_Movie.tjs calls
 * Plugins.link("layerExBtoA.dll") (note the lowercase "toA" -- that is the
 * exact spelling the game's own script uses, and module names are matched
 * as an exact string here, not case-insensitively) to reach
 * copyBottomBlueToTopAlpha/copyRightBlueToLeftAlpha.
 *
 * This is a straight port: unlike layerExRaster/layerExImage, the original
 * source never used the wamsoft layerExBase helper -- it reads the Layer
 * class's own TJS-exposed mainImageBuffer/mainImageBufferForWrite/
 * provinceImageBuffer* properties directly via PropGet on the "Layer"
 * native class, which this engine still exposes the same way (see
 * LayerIntf.cpp's mainImageBuffer/mainImageBufferForWrite/provinceImage*
 * TJS_BEGIN_NATIVE_PROP_DECL blocks). Those buffers are only non-null under
 * TVPIsSoftwareRenderManager(), same caveat as layerExMovie/layerExRaster;
 * every function here already treats a null buffer as failure and throws,
 * matching the original's behavior.
 *
 * The only structural change is defining TJS_PARAM_EXIST locally: the
 * original relied on wamsoft's own extended tp_stub.h for that macro, which
 * isn't part of this engine's tp_stub.h.
 */
...
// レイヤクラスを参照
// 静的リンク時に他プラグイン (layerExSave 等) の同名グローバル関数と
// 多重定義衝突するためファイルローカル (internal linkage) にする。
