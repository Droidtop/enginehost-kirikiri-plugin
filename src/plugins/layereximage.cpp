/*
 * layerExImage.dll: wamsoft's Layer pixel-processing methods -- light
 * (brightness/contrast), colorize, modulate (hue/saturation/luminance),
 * noise, generateWhiteNoise, gaussianBlur.
 *
 * Ported from wamsoft's layerExImage plugin (Main.cpp + LayerExImage.h/.cpp),
 * license "same as KiriKiri itself" per its readme.txt. Noble Works'
 * data/system/LayerEx.tjs calls Plugins.link("layerExImage.dll")
 * unconditionally.
 *
 * The gaussianBlur/colorize/modulate math in LayerExImage.cpp traces back to
 * wamsoft's own adaptation of CxImage's DSP routines (Copyright (C) 2001-2011
 * Davide Pizzolato, "free to use ... for any purpose including commercial
 * applications" per its LICENSE.TXT); this port carries that lineage forward
 * in this comment instead of standing up the original plugin's separate
 * license-manifest/registration mechanism (LicensesGen.cpp), which has no
 * counterpart in this engine.
 *
 * Two structural changes from the original:
 *  - RGBQUAD (a Windows GDI type) doesn't exist in this build (this engine's
 *    win32type.h, which would have declared it, is `#if 0`'d out entirely
 *    and isn't even on this plugin dir's include path), so it's declared
 *    locally here with the same B/G/R/reserved byte layout the code expects.
 *  - The original layerExBase read the layer's pixel buffer through the
 *    "mainImageBufferForWrite" TJS property. This tree's own
 *    src/plugins/layerExBase.hpp (already used by layerExMovie.cpp, and by
 *    layerExRaster.cpp in this same batch) drops that property-based access
 *    as unreliable and exposes layerExBase_GL instead, reaching the pixel
 *    buffer through the native tTJSNI_Layer instance. This port uses
 *    layerExBase_GL in place of layerExBase; every pixel algorithm is
 *    otherwise unchanged. As with layerExMovie/layerExRaster, the buffer is
 *    only populated under TVPIsSoftwareRenderManager(); each method here
 *    bails out harmlessly (no-op) when it is null.
 */
...
#ifndef _WIN32
// Windows GDI type this plugin's original CxImage-derived code expects;
// not available in this build (see comment above), so declared locally.
...
layerExImage::reset()
{
	...
	if (!_buffer) return; // hardware-rendered layer: nothing to process
