/*
 * scriptsEx.dll: wamsoft's "back door" additions to the built-in Scripts
 * class (getObjectKeys/getObjectCount/foreach/clone/propSet/propGet/
 * safeEvalStorage/stringFuzzySearch/rehash and friends).
 *
 * Ported from wamsoft's scriptsEx plugin (Main.cpp + bitap_fuzzy.hpp),
 * license "same as KiriKiri itself" per the plugin's own readme.txt.
 * Noble Works' shared scripts (patch3/utils.tjs, system/Utils.tjs) call
 * Plugins.link("scriptsEx.dll") unconditionally at startup, so without this
 * module every script that pulls in Utils.tjs fails before the game can
 * even show a title screen -- this is the immediate blocker for the game.
 *
 * Changes from the original wamsoft source:
 *  - #include "ncbind.hpp" -> "ncbind/ncbind.hpp" (path layout here).
 *  - MD5 comes from PluginImpl.h's TVP_md5_init/append/finish (same API
 *    the original plugin used, just declared in a different header here).
 *  - TJSDoRehash comes from tjsObject.h.
 *  - propSet/propGet called the plugin's own Try_iTJSDispatch2_PropSet/
 *    PropGet wrappers (SEH guards around the raw iTJSDispatch2 call,
 *    defined in wamsoft's shared tp_stub.cpp). This engine doesn't carry
 *    that SEH scaffolding, and no other internal plugin here uses it
 *    (csvParser.cpp etc. call iTJSDispatch2 methods directly), so this
 *    port calls PropSet/PropSetByNum/PropGet/PropGetByNum directly on the
 *    target object; TJS exceptions still propagate normally either way.
 * Everything else (structure compare/clone, foreach, bitap fuzzy search,
 * safeEvalStorage) is a straight port of the original logic.
 */
...
// フラグ指定つきプロパティ操作
//
// The original wamsoft plugin routed these through Try_iTJSDispatch2_PropSet/
// PropGet (SEH-guarded wrappers from its shared tp_stub.cpp). That scaffolding
// isn't part of this engine and no other internal plugin here uses it, so this
// calls PropSet/PropSetByNum/PropGet/PropGetByNum directly; TJS-level errors
// still propagate the same way (via the tjs_error return / thrown exception).
tjs_error TJS_INTF_METHOD
