/*
 * shrinkCopy.dll: wamsoft's Layer.shrinkCopy (area-averaging downscale
 * blit) and Layer.shrinkCopyFast (fixed-step box-filter downscale used for
 * cheap thumbnails).
 *
 * Ported from wamsoft's shrinkCopy plugin (main.cpp), license "same as
 * KiriKiri itself" per its readme.txt. Noble Works' KAGEnvCharacterEx.tjs
 * links this ("shrinkCopy.dll") to shrink face/CG layers, and several other
 * shared scripts (BitmapFont.tjs, AffineLayer.tjs, PreRenderFontEx.tjs,
 * psdlayer.tjs) call layer.shrinkCopy(...) unconditionally.
 *
 * Straight port: this plugin reads the Layer class's own TJS-exposed
 * mainImageBuffer/mainImageBufferForWrite/mainImageBufferPitch properties
 * directly (no dependency on the wamsoft layerExBase helper), which this
 * engine still exposes the same way. Only change from the original is the
 * include path (ncbind.hpp -> ncbind/ncbind.hpp) and adding NCB_MODULE_NAME.
 */
